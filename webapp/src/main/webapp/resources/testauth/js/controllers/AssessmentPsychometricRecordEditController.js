...
testauth.controller('AssessmentPsychometricRecordEditController', ['$scope', '$state', 'formAction', 'loadedData', 'loadedRecord', 'loadedFileGroup', 'loadedPublishingRecord', 'FileGroupService', 'uploadManager', '$timeout',
	function ($scope, $state, formAction, loadedData, loadedRecord, loadedFileGroup, loadedPublishingRecord, FileGroupService, uploadManager, $timeout) {
		$scope.errors = [];
		$scope.messages = [];
		...
		$scope.back = function() {
            ...
            $state.transitionTo("assessmenthome.psychometric", params);
		};
    	...
    	$scope.save = function() {
    		...
    		$scope.fileRecord.publishingRecordId = $scope.publishingRecord.id;
    		
        	return FileGroupService.savePsychometricRecord($scope.fileRecord, $scope.publishingRecord).then( function(response) {
                $scope.errors = response.errors;
				$scope.messages = response.messages;
				if ($scope.errors.length == 0) {
                    $scope.fileRecord = response.data;
                    $scope.fileGroup.psychometricRecordId = $scope.fileRecord.id;
    	...
    	function saveFileGroup(fileGroup) {
    		return FileGroupService.save(fileGroup).then( function(groupResponse) {
				...
				if ($scope.errors.length == 0) {
                	...
                	$scope.editableForm.$setPristine();
                	$scope.isNew = false;
                	$state.transitionTo("assessmenthome.psychometricForm", { assessmentId : $scope.assessment.id, fileGroupId: $scope.fileGroup.id, formAction: 'Edit'});
				} else {
					angular.forEach($scope.errorsMap, function(msg,key){
						var fieldName = key + "";
						var message = msg + "";
